...
def plot_data(data_dir):
    ...
    # ax.fill_between(np.squeeze(xi_cont), lower_bound_abs, upper_bound_abs, color='blue', alpha=0.2, label='Prediction Std Dev')
    ...
    upper_bound_real = np.real(GP_FT_mu) + np.real(GP_FT_stdv)
    lower_bound_real = np.real(GP_FT_mu) - np.real(GP_FT_stdv)
    ax = plt.subplot2grid((5, 3), (3, 2))
    ax.plot(xi_cont, np.real(GP_FT_mu), label='Prediction Mean', color='red')
    ax.fill_between(np.squeeze(xi_cont), lower_bound_real, upper_bound_real, color='blue', alpha=0.2, label='Prediction Std Dev')
    ax.set_xlabel('Angular Frequency [rad s$^{-1}$]')
    ax.set_ylabel('(a) Real Part [ms$^{-2}$]')
    ax.set_title('GP Fourier Transform')
    ...
    upper_bound_imag = np.imag(GP_FT_mu) + np.real(GP_FT_stdv)
    lower_bound_imag = np.imag(GP_FT_mu) - np.real(GP_FT_stdv)
    ax = plt.subplot2grid((5, 3), (4, 2))
    ax.plot(xi_cont, np.imag(GP_FT_mu), label='Prediction Mean', color='red')
    ax.fill_between(np.squeeze(xi_cont), lower_bound_imag, upper_bound_imag, color='blue', alpha=0.2, label='Prediction Std Dev')
    ax.set_xlabel('Angular Frequency [rad s$^{-1}$]')
    ax.set_ylabel('(b) Imaginary Part [ms$^{-2}$]')
    # ax.set_title('GP Fourier Transform')
    max_imag = 1.1 * max(np.max(np.abs(np.imag(analytical_FT))), np.max(np.abs(np.imag(DFT))), np.max(np.abs(np.imag(GP_FT_mu))))
    ax.set_ylim(-max_imag, max_imag)
    ax.legend(fontsize='small')
